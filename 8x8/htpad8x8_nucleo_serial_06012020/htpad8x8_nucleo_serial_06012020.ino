...
void read_pixel_data() {
  ...
  // |  0  |  0  |  0  |  0  |   1   |   0   |   0   |    1   |
  write_sensor_byte(SENSOR_ADDRESS, CONFIGURATION_REGISTER, 0x09 );
  //Wire.beginTransmission(SENSOR_ADDRESS);  Wire.write(CONFIGURATION_REGISTER);  Wire.write(0x09);  Wire.endTransmission();
  ...
  Wire.beginTransmission(SENSOR_ADDRESS);  Wire.write(CONFIGURATION_REGISTER);  Wire.write(0x0B + 0x04);  Wire.endTransmission();
...
void write_calibration_settings_to_sensor() {
  ...
  Wire.beginTransmission(SENSOR_ADDRESS);  Wire.write(TRIM_REGISTER1);  Wire.write(mbit_calib);  Wire.endTransmission(); delay(5);
  Wire.beginTransmission(SENSOR_ADDRESS);  Wire.write(TRIM_REGISTER2);  Wire.write(bias_calib);  Wire.endTransmission(); delay(5);
  Wire.beginTransmission(SENSOR_ADDRESS);  Wire.write(TRIM_REGISTER3);  Wire.write(bias_calib);  Wire.endTransmission(); delay(5);
  Wire.beginTransmission(SENSOR_ADDRESS);  Wire.write(TRIM_REGISTER4);  Wire.write(clk_calib);  Wire.endTransmission(); delay(5);
  Wire.beginTransmission(SENSOR_ADDRESS);  Wire.write(TRIM_REGISTER5);  Wire.write(bpa_calib);  Wire.endTransmission(); delay(5);
  Wire.beginTransmission(SENSOR_ADDRESS);  Wire.write(TRIM_REGISTER6);  Wire.write(bpa_calib);  Wire.endTransmission(); delay(5);
  Wire.beginTransmission(SENSOR_ADDRESS);  Wire.write(TRIM_REGISTER7);  Wire.write(pu_calib);  Wire.endTransmission(); delay(5);
...
void receive_eeprom_data( uint16_t cmd, uint8_t *dest, uint16_t n) {
  ...
  Wire.requestFrom(EEPROM_ADDRESS, n, cmd, 1, 1);
  while (Wire.available()) {
    *dest++  = Wire.read();
  }
  Wire.endTransmission();
...
/********************************************************************
   ...
   Description:     write eeprom register (see datasheet, chp.: 10.07 I2C Example Sequence – EEPROM Erase/Write )
