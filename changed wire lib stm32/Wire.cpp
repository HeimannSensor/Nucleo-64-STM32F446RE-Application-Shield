...
uint16_t TwoWire::rxBufferAllocated = 0;	// dennis: uint8_t -> uint16_t
uint16_t TwoWire::rxBufferIndex = 0;	// dennis: uint8_t -> uint16_t
uint16_t TwoWire::rxBufferLength = 0;	// dennis: uint8_t -> uint16_t
...
uint8_t TwoWire::requestFrom(uint8_t address, uint16_t quantity, uint32_t iaddress, uint8_t isize, uint8_t sendStop)
{

#if !defined(I2C_OTHER_FRAME)
  UNUSED(sendStop);
#endif
  uint16_t read = 0;	// dennis: uint8_t -> uint16_t
...
/**
  ...
  */
void TwoWire::allocateRxBuffer(uint16_t length)
{
