...
void calculate_pixel_temp() {
  ...
  for (int m = 0; m < sensor.number_row; m++) {
    for (int n = 0; n < sensor.number_col; n++) {
      ...
      comp_step = ((int32_t)data_pixel[m][n] - (int32_t)( (int32_t)thgrad[m][n] * (int32_t)ptat_av_uint16) / (int32_t)gradscale_div - (int32_t)thoffset[m][n]);
      ...
      if (m < sensor.number_row / 2) {
        comp_step = comp_step - eloffset[m % 8][n];
      }
      // bottom half
      else {
        comp_step = comp_step - eloffset[m % 8 + 8][n];
      }
      ...
      comp_step =  (vij_pixc_and_pcscaleval / pixcij);
      ...
      table_row = comp_step + TABLEOFFSET;
      ...
      comp_step = (uint32_t)((vy - vx) * ((int32_t)(comp_step + TABLEOFFSET) - (int32_t)YADValues[table_row]) / ydist + (int32_t)vx);

      // --- GLOBAL OFFSET ---
      data_pixel[m][n] = comp_step + globaloff;
...
/********************************************************************
   ...
   Dependencies:    register address (addr),
                    number of bytes (n)
 *******************************************************************/
void write_sensor_byte( uint8_t addr, uint8_t input) {
